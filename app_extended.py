...
if os.path.exists(CACHE_PATH):
    ...
    # Use the team member's CovidDataPreprocessor for cleaning (same as app.py)
...
TABS = [
    ...
    # ── Advanced dropdown ──
    {'label': 'Advanced ▾', 'value': 'tab-advanced'},
]
ADVANCED_SUB_TABS = [
    ...
    html.Div([
        dcc.Tabs(id='main-tabs', value='tab-overview',
                 children=[dcc.Tab(label=t['label'], value=t['value'],
                                   ...
                                   selected_style={'fontSize': '11px', 'padding': '6px 10px',
                                                   'fontWeight': '700', 'borderTop': '2px solid #2a4d8f'})
                           for t in TABS],
                 style={'fontSize': '11px', 'fontWeight': '500'}),
    ], style={'overflowX': 'auto', 'overflowY': 'hidden', 'whiteSpace': 'nowrap',
              'background': '#fff', 'borderBottom': '1px solid #e8e8e8'}),

    html.Div(id='advanced-sub-bar', style={'display': 'none', 'padding': '4px 24px',
                                            'background': '#f0f4fa', 'borderBottom': '1px solid #d0d8e4',
...
def build_pipeline():
    """Data Pipeline tab: shows data loading and cleaning steps (same as app.py)."""
...
def build_overview(metric):
    """Overview tab: dataset info + global summary + country summary table."""
    ...
    # Pre-compute country summary for initial render
    ...
    return html.Div([
        ...
        html.Div([
            ...
            stat_card('Global Total', f'{latest[metric].sum():,.0f}', metric_label),
                  ...
                  'flexWrap': 'wrap'}),
        # Key metrics summary (dynamic: linked to table filter)
        html.Div([
            ...
            html.Div(id='overview-key-metrics', children=[
                ...
                html.Div([
                  ...
                  'border': '1px solid #e8e8e8', 'marginBottom': '12px'}),
    # Country summary table with filter
    html.Div([
        html.Div([
            html.Span('Country Summary', style={'fontSize': '14px', 'fontWeight': '600',
                     'color': '#444'}),
            html.Div([
                ...
                dcc.DatePickerRange(
                  ...
                  'marginBottom': '8px', 'borderBottom': '1px solid #e8e8e8', 'paddingBottom': '8px'}),
            html.Div([
                dash_table.DataTable(
                    id='data-table',
                    columns=[
                        ...
                        {'name': 'Vaccinated %', 'id': 'people_fully_vaccinated_per_hundred'},
                        {'name': 'New Cases (latest)', 'id': 'new_cases_smoothed'},
                        {'name': 'New Deaths (latest)', 'id': 'new_deaths_smoothed'},
                    ],
                    data=country_summary.head(100).to_dict('records'),
                    page_size=10,
                    style_table={'overflowX': 'auto'},
                    style_cell={
                        'fontSize': '12px', 'fontFamily': 'Arial',
                    ...
                    style_data={'borderBottom': '1px solid #f0f0f0'},
                    style_cell_conditional=[
                        {'if': {'column_id': 'total_cases'}, 'textAlign': 'right'},
                        {'if': {'column_id': 'total_deaths'}, 'textAlign': 'right'},
                        {'if': {'column_id': 'new_cases_smoothed'}, 'textAlign': 'right'},
                        {'if': {'column_id': 'new_deaths_smoothed'}, 'textAlign': 'right'},
                    ]
                )
...
def build_global(metric, compare_list=None):
    ...
    return html.Div([
        ...
        # Timeline slider with date label and play button (same as app.py)
        html.Div([
...
# ═══════════════════════════════════════════════════════════════════════
# Original Global callbacks (same as app.py)
# ═══════════════════════════════════════════════════════════════════════
@app.callback(
...
# ═══════════════════════════════════════════════════════════════════════
# Advanced sub-bar visibility & auto-redirect
# ═══════════════════════════════════════════════════════════════════════
@app.callback(
    [Output('advanced-sub-bar', 'children'),
     Output('advanced-sub-bar', 'style'),
     Output('main-tabs', 'value')],
    Input('main-tabs', 'value')
)
def handle_advanced_tab(tab):
    if tab == 'tab-advanced':
        # Show sub-bar and redirect to first advanced tab
        sub_buttons = []
        for i, st in enumerate(ADVANCED_SUB_TABS):
            btn = html.Span(
                st['label'],
                id={'type': 'adv-btn', 'index': i},
                n_clicks=0,
                style={'cursor': 'pointer', 'padding': '4px 14px',
                       'borderRadius': '12px', 'background': '#dce4f0',
                       'color': '#2a4d8f', 'fontSize': '12px',
                       'fontWeight': '500', 'display': 'inline-block'}
            )
            sub_buttons.append(btn)
        return (
            [html.Span('Advanced: ', style={'fontWeight': '600', 'color': '#2a4d8f', 'marginRight': '8px'}),
             html.Div(sub_buttons, style={'display': 'flex', 'gap': '6px'})],
            {'display': 'flex', 'alignItems': 'center', 'padding': '4px 24px',
             'background': '#f0f4fa', 'borderBottom': '1px solid #d0d8e4'},
            ADVANCED_SUB_TABS[0]['value']
        )
    else:
        return [], {'display': 'none'}, tab

# ═══════════════════════════════════════════════════════════════════════
# Advanced sub-tab button clicks → switch tab
# ═══════════════════════════════════════════════════════════════════════
@app.callback(
    Output('main-tabs', 'value', allow_duplicate=True),
    Input({'type': 'adv-btn', 'index': dash.dependencies.ALL}, 'n_clicks'),
    prevent_initial_call=True
)
def adv_btn_click(n_clicks_list):
    ...
    return ADVANCED_SUB_TABS[idx]['value']

# ═══════════════════════════════════════════════════════════════════════
# Overview: filter data table by country (summary vs detail)
# ═══════════════════════════════════════════════════════════════════════
@app.callback(
...
def filter_overview_table(country, start_date, end_date):
    if country == 'ALL':
        # Country summary: one row per country with totals
        summary = df.loc[df.groupby('country')['date'].idxmax(),
        ...
        summary = summary.sort_values('total_cases', ascending=False).head(100)
        cols = [
            {'name': 'Country', 'id': 'country'},
            {'name': 'Continent', 'id': 'continent'},
            {'name': 'Total Cases', 'id': 'total_cases'},
        ...
        return summary.to_dict('records'), cols, metrics
    else:
        # Detail view: daily rows for selected country within date range
        cdf = df[df['country'] == country].copy()
        if start_date:
            cdf = cdf[cdf['date'] >= start_date]
        if end_date:
            cdf = cdf[cdf['date'] <= end_date]
        cdf = cdf[['country', 'date', 'continent',
                   'new_cases_smoothed', 'new_deaths_smoothed',
                   'people_fully_vaccinated_per_hundred']].head(500)
        cols = [
            {'name': 'Country', 'id': 'country'},
            {'name': 'Date', 'id': 'date'},
            {'name': 'Continent', 'id': 'continent'},
            {'name': 'New Cases', 'id': 'new_cases_smoothed'},
            {'name': 'New Deaths', 'id': 'new_deaths_smoothed'},
            {'name': 'Vaccinated %', 'id': 'people_fully_vaccinated_per_hundred'},
        ]
        # Country-specific metrics
        c_latest = df[df['country'] == country].iloc[-1] if len(df[df['country'] == country]) > 0 else None
        if c_latest is not None:
            total_cases = int(c_latest['total_cases']) if pd.notna(c_latest['total_cases']) else 0
            total_deaths = int(c_latest['total_deaths']) if pd.notna(c_latest['total_deaths']) else 0
            total_vacc = int(c_latest['people_fully_vaccinated']) if pd.notna(c_latest['people_fully_vaccinated']) else 0
            vacc_rate = round(c_latest['people_fully_vaccinated_per_hundred'], 1) if pd.notna(c_latest['people_fully_vaccinated_per_hundred']) else 0
        else:
            total_cases = total_deaths = total_vacc = 0
            vacc_rate = 0
        metrics = [
            html.Div([
                html.Div('Total Cases', style={'fontSize': '11px', 'color': '#888'}),
                html.Div(f'{total_cases:,}', style={'fontSize': '16px', 'fontWeight': '700', 'color': '#2a4d8f'})
            ], style={'flex': '1', 'textAlign': 'center', 'padding': '8px'}),
            html.Div([
                html.Div('Total Deaths', style={'fontSize': '11px', 'color': '#888'}),
                html.Div(f'{total_deaths:,}', style={'fontSize': '16px', 'fontWeight': '700', 'color': '#b91f1f'})
            ], style={'flex': '1', 'textAlign': 'center', 'padding': '8px'}),
            html.Div([
                html.Div('Fully Vaccinated', style={'fontSize': '11px', 'color': '#888'}),
                html.Div(f'{total_vacc:,}', style={'fontSize': '16px', 'fontWeight': '700', 'color': '#1a7a3a'})
            ], style={'flex': '1', 'textAlign': 'center', 'padding': '8px'}),
            html.Div([
                html.Div('Vaccination Rate', style={'fontSize': '11px', 'color': '#888'}),
                html.Div(f'{vacc_rate}%', style={'fontSize': '16px', 'fontWeight': '700', 'color': '#1a7a3a'})
            ], style={'flex': '1', 'textAlign': 'center', 'padding': '8px'}),
        ]
        return cdf.to_dict('records'), cols, metrics
